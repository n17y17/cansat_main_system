#ifndef SC_PROJECT_RP_PICO_PICO_SDK_ARDUINO_EXAMPLE_SC_A_HPP_
#define SC_PROJECT_RP_PICO_PICO_SDK_ARDUINO_EXAMPLE_SC_A_HPP_
...
namespace sc
{
    ...
    // ログを記録する関数です．
    // printfの形式で入力
    template<typename ... Args> void save_log(const std::string& fmt, Args ... args)
    {
        size_t len = std::snprintf( nullptr, 0, fmt.c_str(), args ... );  // フォーマット後の文字数を計算
        char buf[len + 1];  // フォーマット済みの文字列を保存するための配列を作成
    ...
    void save_error(const std::string& FILE, int LINE, const std::string& message);
    ...
    struct A_Direction : Measurement {using Measurement::Measurement;};  // 絶対的(Absolute)な方角 (°)  水平面における角度  0°:北, 90°:東, 180°:南, 270°:西
    ...
    struct Longitude : Measurement {using Measurement::Measurement;};  // 経度 (ddd.dd...°)  正:東経, 負:西経
    ...
    struct A_Position  // 絶対(Absolute)直交座標上の位置 (m)  電源を入れた位置を原点とし，+y:北, +x:東, +z:天頂 とする右手系
    {
        X x = ErrorValue;
        Y y = ErrorValue;
        Z z = ErrorValue;
    ...
    struct R_Position  // 相対(Relative)直交座標上の位置 (m)  今の機体の位置を原点とし，+y:前, +x:右, +z:上 とする右手系
    {
        X x = ErrorValue;
        Y y = ErrorValue;
        Z z = ErrorValue;
    ...
    struct Acceleration : VectorXYZ {using VectorXYZ::VectorXYZ;};  // 加速度 (m/s/s) (今の機体の位置を原点とし，+y:前, +x:右, +z:上 とする右手系)
    struct GravityAcceleration : Acceleration {using Acceleration::Acceleration;};  // 重力加速度 (m/s/s) (今の機体の位置を原点とし，+y:前, +x:右, +z:上 とする右手系)
    struct LineAcceleration : Acceleration {using Acceleration::Acceleration;};  // 重力を除いた加速度 (m/s/s) (今の機体の位置を原点とし，+y:前, +x:右, +z:上 とする右手系)
    struct Gyro : VectorXYZ {using VectorXYZ::VectorXYZ;};  // 角速度 (rad/s) +x:X軸の右ネジの向き, +y:Y軸の右ネジの向き, +z:Z軸の右ネジの向き (今の機体の位置を原点とし，+y:前, +x:右, +z:上 とする右手系)
    struct Magnetism : VectorXYZ {using VectorXYZ::VectorXYZ;};  // 磁気 (mT) (今の機体の位置を原点とし，+y:前, +x:右, +z:上 とする右手系)

    struct Tilt  // 機体の傾き  pitch:X軸の右ネジの向きへの傾き(°), roll:Y軸の右ネジの向きへの傾き(°), Z軸の右ネジの向きへの傾き(°) (各-180°<, <=180°) (今の機体の位置を原点とし，+y:北, +x:東, +z:天頂 とする右手系)
    {
        X pitch = ErrorValue;
        Y roll = ErrorValue;
        Z yaw = ErrorValue;
        Tilt() = default;
        Tilt(X pitch_, Y roll_, Y yaw_) : pitch(pitch_), roll(roll_), yaw(yaw_) {}
        explicit Tilt(double value) : Tilt(value, value, value) {}
    };
    ...
    struct A_Angle  // 空間における向き (°)  方位に対する絶対的(Absolute)な向き
    {
        ...
        A_Angle() = default;
        A_Angle(A_Direction direction_, A_Elevation elevation_) : direction(direction_), elevation(elevation_) {}
        explicit A_Angle(double value) : A_Angle(value, value) {}
    };
    struct R_Angle  // 空間における向き (°)  機体に対する相対的(Relative)な向き
    {
        ...
        R_Angle() = default;
        R_Angle(R_Direction direction_, R_Elevation elevation_) : direction(direction_), elevation(elevation_) {}
        explicit R_Angle(double value) : R_Angle(value, value) {}
    };
    ...
    public:
        ...
        Altitude0(const Pressure& pressure0, const Temperature& temperature0 = 25.0, const Altitude& altitude0 = 0.0);

        static Pressure pressure0() {return _Pressure0;}
        static Temperature temperature0() {return _Temperature0;}
        static Altitude altitude0() {return _Altitude0;}
    };
    ...
    public:
        ...
        Position0(const Latitude& latitude0, const Longitude& longitude0, const Altitude& altitude0 = 0.0);

        static Latitude latitude0() {return _Latitude0;}
        static Longitude longitude0() {return _Longitude0;}
        static Altitude altitude0() {return _Altitude0;}
    };
    ...
    Altitude to_altitude (const Pressure& pressure, const Temperature& temperature, const Pressure& pressure0 = Altitude0::pressure0(), const Altitude& altitude0 = Altitude0::altitude0());
    ...
    // [altitude0] : 基準点の気圧  (省略時:Altitude0でセットした標高)
    Altitude to_altitude (const Pressure& pressure, const Pressure& pressure0 = Altitude0::pressure0(), const Temperature& temperature0 = Altitude0::temperature0(), const Altitude& altitude0 = Altitude0::altitude0());
    ...
    // [temperature0] : 基準点の気圧  (省略時:Altitude0でセットした気温)
    // [altitude0] : 基準点の気圧  (省略時:Altitude0でセットした標高)
    Altitude to_altitude (const Temperature& temperature, const Temperature& temperature0 = Altitude0::temperature0(), const Altitude& altitude0 = Altitude0::altitude0());
    ...
    Altitude to_altitude(const GravityAcceleration& gravity, const Magnitude& gravity0 = Earth_g, const Altitude& altitude = 0.0);

    // 重力加速度から標高を計算
    // ！非推奨！精度が悪いです
    // gravity : 測定した重力加速度
    // latitude : 測定した地点の緯度
    Altitude to_altitude(const GravityAcceleration& gravity, const Latitude& latitude);
    ...
    A_Position to_position(const Latitude& latitude, const Longitude& longitude, const Altitude& altitude = 0.0, const Latitude& latitude0 = Position0::latitude0(), const Longitude& longitude0 = Position0::longitude0(), const Altitude& altitude0 = Position0::altitude0());
    ...
    // [longitude0] : 原点の経度
    // [altitude0] : 原点の標高
    A_Position to_position_simple(const Latitude& latitude, const Longitude& longitude, const Altitude& altitude = 0.0, const Latitude& latitude0 = Position0::latitude0(), const Longitude& longitude0 = Position0::longitude0(), const Altitude& altitude0 = Position0::altitude0());

    // 2点のXYZ直交座標から直線距離を計算
    // position : XYZ直交座標での位置  1つ目
    // position0 : XYZ直交座標での位置  2つ目
    Distance to_distance(const A_Position& position, const A_Position& position0);
    ...
    Distance to_distance(const Latitude& latitude, const Longitude& longitude, const Latitude& latitude0 = Position0::latitude0(), const Longitude& longitude0 = Position0::longitude0());
    ...
    Distance to_distance_simple(const Latitude& latitude, const Longitude& longitude, const Latitude& latitude0 = Position0::latitude0(), const Longitude& longitude0 = Position0::longitude0());
    // この関数の作成にあたり以下の資料を参考にしました
    ...
    public:
        ...
        virtual Altitude altitude() const {save_error(__FILE__, __LINE__, "This sensor cannot return altitude"); return ErrorValue;}  // このセンサは標高を返すことができません
        virtual Temperature temperature() const {save_error(__FILE__, __LINE__, "This sensor cannot return temperature"); return ErrorValue;}  // このセンサは気温を返すことができません
        virtual Pressure pressure() const {save_error(__FILE__, __LINE__, "This sensor cannot return pressure"); return ErrorValue;}  // このセンサは気圧を返すことができません
        virtual Humidity humidity() const {save_error(__FILE__, __LINE__, "This sensor cannot return humidity"); return ErrorValue;}  // このセンサは湿度を返すことができません
        virtual Latitude latitude() const {save_error(__FILE__, __LINE__, "This sensor cannot return latitude"); return ErrorValue;}  // このセンサは緯度を返すことができません
        virtual Longitude longitude() const {save_error(__FILE__, __LINE__, "This sensor cannot return longitude"); return ErrorValue;}  // このセンサは経度を返すことができません
        virtual Distance distance() const {save_error(__FILE__, __LINE__, "This sensor cannot return distance"); return ErrorValue;}  // このセンサは距離を返すことができません
        virtual Area area() const {save_error(__FILE__, __LINE__, "This sensor cannot return area"); return ErrorValue;}  // このセンサは面積を返すことができません
        virtual Acceleration acceleration() const {save_error(__FILE__, __LINE__, "This sensor cannot return acceleration"); return Acceleration(ErrorValue);}  // このセンサは加速度を返すことができません
        virtual Gyro gyro() const {save_error(__FILE__, __LINE__, "This sensor cannot return gyro"); return Gyro(ErrorValue);}  // このセンサは角速度を返すことができません
        virtual Magnetism magnetism() const {save_error(__FILE__, __LINE__, "This sensor cannot return magnetism"); return Magnetism(ErrorValue);}  // このセンサは磁気を返すことができません
    };
...
#endif  // SC_PROJECT_RP_PICO_PICO_SDK_ARDUINO_EXAMPLE_SC_A_HPP_