...
namespace sc
{
    ...
    void BME280::init(uint8_t select_device, Communication* i2c_or_spi)
    {
        ...
        if (!check_connection()) 
        {
            save_error(__FILE__, __LINE__, "BME280 could not be set up correctly");  // BME280を正常にセットアップできませんでした
        }
    ...
    bool BME280::check_connection()
    {
        if (_i2c_or_spi == nullptr)
        {
            save_error(__FILE__, __LINE__, "BME280 cannot be used until setup is complete");  // BME280はセットアップが完了するまで使用できません
    return false;
        }
        ...
        switch (chip_id)
        {
            case 0x60:
            {
                sc::save_log("BME280 is connected normally"); // BME280は正常に接続されています
    return true;
            }
            case 0x58:
            {
                sc::save_log("BMP280 is connected instead of BME280; use the BMP280-specific program.");  // BME280ではなくBMP280が接続されています．BMP280専用のプログラムを使用してください．
    return true;
            }
            default:
            {
                sc::save_log("BME280 connection could not be verified");  // BME280の接続が確認できませんでした
    return false;
            }
        }
    }
    ...
    // 測定した気温を返す
    Temperature BME280::temperature() const
    {
        return _temperature;
    }

    // 測定した気圧を返す
    Pressure BME280::pressure() const
    {
        return _pressure;
    }

    // 測定した湿度を返す
    Humidity BME280::humidity() const
    {
        return _humidity;
    }
    ...
    void BME280::set_parameter(Mode mode, OverSampling over_sampling_t, OverSampling over_sampling_p, OverSampling over_sampling_h, StanbyTime stanby_time, Filter filter, SpiWireNum spi_wire_num) const
    {
        if (_i2c_or_spi == nullptr)
        {
            save_error(__FILE__, __LINE__, "BME280 cannot be used until setup is complete");  // BME280はセットアップが完了するまで使用できません
    ...
    // 補正用のパラメータを計算
    void BME280::calc_calibration_parameter()
    {
        _calibration_parameter = (((((_raw_temperature >> 3) - ((int32_t) dig_T1 << 1))) * ((int32_t) dig_T2)) >> 11) + ((((((_raw_temperature >> 4) - ((int32_t) dig_T1)) * ((_raw_temperature >> 4) - ((int32_t) dig_T1))) >> 12) * ((int32_t) dig_T3)) >> 14);
    }

    // 気温データを補正して保存
    void BME280::calibrate_temperature()
    {
        _temperature = ((_calibration_parameter * 5 + 128) >> 8) / 100.0;
    }

    // 気圧データを補正して保存
    void BME280::calibrate_pressure()
    {
        int32_t var1, var2;
        uint32_t p;
        var1 = (((int32_t) _calibration_parameter) >> 1) - (int32_t) 64000;
    ...
    // 湿度データを補正して保存
    void BME280::calibrate_humidity()
    {
        int32_t v_x1_u32r;
        v_x1_u32r = (_calibration_parameter - ((int32_t) 76800));
